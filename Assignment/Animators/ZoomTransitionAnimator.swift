...
/**
 Contains functions to handle animating a zoom in/out transition on
   navigation controller push events
 */
class ZoomTransitionAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        var initialFrame = presenting ? frame : detailView.frame
        let finalFrame = presenting ? detailView.frame : frame
        ...
        let storyDetailAspectRatio = detailView.frame.width / detailView.frame.height
        ...
        if presenting {
            ...
            detailView.center = CGPoint(x: frame.midX, y: frame.midY)
        ...
        UIView.animate(withDuration: duration, delay: 0.0, usingSpringWithDamping: 1, initialSpringVelocity: 1.0, options: .curveEaseInOut, animations: {
            ...
            if presenting {
                ...
                let translate = mainView.transform.translatedBy(x: mainView.frame.midX - self.frame.midX, y: mainView.frame.midY - self.frame.midY)
                mainView.transform = translate.concatenating(scale)
